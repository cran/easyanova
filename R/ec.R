...
function(mg1,mg2,sdg1,sdg2, gl){
    ...
    p=(1-pt(tcal,gl))*2; grupos="group.1 vs group.2"
    res=data.frame(grupos, contrast=round(y,2),standart.error=round(sd,4), tcal=round(tcal,2), p.value=round(p,4))
    return(res)
}
