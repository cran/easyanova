...
Adjust.Means<-fixef(m1)[1:nlevels(data$factor_1)]
...
rownames(maf1)=NULL
Adjust.Means<-fixef(m2) [1:nlevels(data$factor_2)]
...
rownames(maf2)=NULL
Adjust.Means<-fixef(m3) [1:nlevels(data$factor_3)]
...
rownames(maf3)=NULL
Adjust.Means<-fixef(m4)[1:nlevels(data$treatments_f1f2)]
...
rownames(mat1)=NULL
Adjust.Means<-fixef(m5)[1:nlevels(data$treatments_f1f3)]
...
rownames(mat2)=NULL
Adjust.Means<-fixef(m6)[1:nlevels(data$treatments_f2f3)]
...
rownames(mat3)=NULL
Adjust.Means<-fixef(m7)[1:nlevels(data$treatments_f1f2f3)]
...
rownames(mat4)=NULL
test1<-summary(kp(glht (m1,  linfct = mcp(factor_1 = "Tukey"),df=df1)))
cld(test1)
test2<-summary(kp(glht (m2,  linfct = mcp(factor_2 = "Tukey"), df=df1)))
cld(test2)
test3<-summary(kp(glht (m3,  linfct = mcp(factor_3 = "Tukey"), df=df2)))
cld(test3)
...
r1=lapply(c1, function(c1i) { summary(glht(m4,  linfct =c1i, df=df1)) })
r2=lapply(c2, function(c2i) { summary(glht(m4,  linfct =c2i, df=df1)) })
...
r3=lapply(c1, function(c1i) { summary(glht(m5,  linfct =c1i, df=df2)) })
r4=lapply(c2, function(c2i) { summary(glht(m5,  linfct =c2i, df=df2)) })
...
r5=lapply(c1, function(c1i) { summary(glht(m6,  linfct =c1i, df=df2)) })
r6=lapply(c2, function(c2i) { summary(glht(m6,  linfct =c2i, df=df2)) })
...
r7=lapply(c2, function(c2i) { summary(glht(m7,  linfct =c2i, df=df2)) })
...
r8=lapply(c2, function(c2i) { summary(glht(m8,  linfct =c2i, df=df2)) })
...
r9=lapply(c2, function(c2i) { summary(glht(m9,  linfct =c2i, df=df2)) })
ai<-as.numeric(AIC(m)); bi<-as.numeric(BIC(m)); log=as.numeric(logLik(m)); 
qa=data.frame(ai,bi,log)
colnames(qa)<-c("AIC","BIC", "LogLik")
rownames(qa)<-NULL
l<-list(a3,s,qa, maf1, maf2, maf3, mat1, mat2, mat3, mat4, test1,test2, test3, r2, r1, r4,r3,r6,r5,r7,r8,r9)
names(l)= list("Analysis of variance", "Normality test ", "Parameters of model fitting","Adjusted means (factor 1)", "Adjusted means (factor 2)", "Adjusted means (factor 3)", "Adjusted means (treatments_f1f2)", "Adjusted means (treatments_f1f3)", "Adjusted means (treatments_f2f3)", "Adjusted means (treatments_f1f2f3)","Multiple comparison test (factor 1)", "Multiple comparison test (factor 2)", "Multiple comparison test (factor 3)","Multiple comparison test (factor 1 in levels of factor 2)", "Multiple comparison test (factor 2 in levels of factor 1)","Multiple comparison test (factor 1 in levels of factor 3)", "Multiple comparison test (factor 3 in levels of factor 1)", "Multiple comparison test (factor 2 in levels of factor 3)","Multiple comparison test (factor 3 in levels of factor 2)","Multiple comparison test (factor 1 in levels of treatments factor 2 * factor 3)","Multiple comparison test (factor 2 in levels of treatments factor 1 * factor 3)", "Multiple comparison test (factor 3 in levels of treatments factor 1 * factor 2)")
return(l)
}
,
f2<-function(data){ 
